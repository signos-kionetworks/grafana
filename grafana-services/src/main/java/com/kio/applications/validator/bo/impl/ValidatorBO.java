...
public class ValidatorBO implements IfzValidatorBO, Serializable {
    ...
    public ValidatorResponse processRequest(ValidatorRequest value) throws GenericException {
        ...
        indicator.setMantime((double) value.getExtraVars().getManualTime());
        ...
        if (!value.getExtraVars().getOrigin().trim().isEmpty()) {
            if (value.getExtraVars().getResourceId().trim().isEmpty()) {
                throw new GenericException("El campo resource_id es obligatorio cuando el campo origin es enviado.");
            }
            if (value.getExtraVars().getResourceName().trim().isEmpty()) {
                throw new GenericException("El campo resource_name es obligatorio cuando el campo origin es enviado.");
            }
            Origin origin = originBO.selectByName(value.getExtraVars().getOrigin().trim());
            if (origin == null) {
                throw new GenericException(String.format("El valor [%s] del campo origin no existe.",
                        value.getExtraVars().getOrigin().trim()));
            }

            Resource resource = resourceBO.searchResourceByResourceId(
                    value.getExtraVars().getResourceId().trim(), origin.getId());

            if(null == resource){
                resource = new Resource();
                resource.setIdOrigen(origin.getId());
                resource.setIdentificadorRecurso(value.getExtraVars().getResourceId().trim());
            }
            resource.setName(value.getExtraVars().getResourceName().trim().toUpperCase());

            resourceBO.save(resource);

            indicator.setIdRecurso(resource.getId());
        }
    ...
    private Integer validateLevelOfSpecialization(ValidatorRequest value) throws GenericException {
        LevelOfSpecialization levelOfSpecialization;
     ...
     * @throws GenericException the generic exception
     */
    private void validateRequest(ValidatorRequest value) throws GenericException {
        if (null == value) {
    ...
    private Integer validateTechnologicalDomain(ValidatorRequest value) throws GenericException {
        TechnologicalDomain technologicalDomain;
